...
## Stage2 research completed (2026-09-17)

All postprocessing completed successfully:35,688 stress rows,336 mitigation rows, full integrity verification, figures and fresh sequential Windows timing. Reference/evaluator median planning-time ratios are1328.6 and1389.1 for predeclared h101/h102; no mixed-host or inference-speedup claim. Four selected explanatory traces were added after timing finished; each exactly matches original per-query metrics and final simulation time. The inherited conservative trace was reused. A diagnostic evaluator-only fixed-overhead progress ablation demonstrates overlap sensitivity, without changing nominal labels or baseline SHA. Nominal six Decode-tight winners agree and six Prefill-oriented winners disagree (8.30–26.16% sampled capacity regret). Large-gap/correct-winner evidence is explicitly from Both−5%/−10% stress, not nominal. Quality means exclude two zero-oracle cases (26/28 defined); unsafe/abstention counts retain28. The14-section Chinese FORMAL_EVIDENCE_REPORT.md, mechanism analysis and claim boundaries are complete;28 tests and a refreshed formal verification pass. Figures were visually reviewed and cropping/window-selection issues corrected. No additional frozen experiments are required. Automatic monitoring is paused after final delivery.

## Stage 3 — Final evaluator-only sensitivity and evidence freeze (2026-09-17)

Read the complete Stage2 evidence, audit, protocol, mechanisms and outputs. Ran3268 legacy-progress evaluator calls on all76 nominal configuration/regime common grids; reused1634 physical reference records and recomputed labels from saved request metrics. Added only5ms to progress with zero queue asserted; no source archaeology, grid expansion, baseline changes or reference runs. Snapshotted2387 protected files and verified byte preservation. The existing Python launcher initially failed under sandbox access restrictions; the same installed environment ran successfully under approved execution, without changing dependencies.

Raw→legacy: O23→17, C413→460;53 safe→unsafe label changes. Tight winners6/6 persist; relaxed0/6→2/6. Raw4 strict (h101–h104)+2 tie-break (h105/h106); legacy3 strict (h101–h103)+1 tie-break (h106). h104/h105 improve; h102 rank2→4 and loss8.30%→14.07%. Interpretation: Yes, but modified, preserving partial implementation sensitivity and residual failures.

Decomposition records24 raw/control main rows plus144 existing mitigation main outputs. Raw relaxed partition loss8.30%–26.16% versus operating-point loss68.95%–95.86%; h106 exact output is R-unsafe, safe usable loss100%. Audit covers all56 derating cases;54 have defined realized reduction. Nominal realized median46.53%, max79.87%; h101/tight example59.74%. Snapping and reapplying tie-break changes8 nominal partitions. The5-query validation result is conditional replay after reference-informed grid construction, not planning from scratch.

Updated core figures/SLA labels, strict/tie annotations, claim boundary, formal report, outline and18-section Stage3 writing handoff. Existing35,688 stress/336 mitigation rows and five mechanism traces remain unchanged.31 tests pass; formal consistency, source protection, denominators, figure/table inputs and report links are checked. No further experiments needed for these limited claims; no manuscript prose drafted.

## Stage 4 — SLA-regime sensitivity (2026-09-17)

Source HEAD exactly69a6b175b5be6788b6448f23edbf1366c1f626c3, initially clean. Read the required Stage3 evidence/protocol/code and inspected formal results. Froze config/sla_sensitivity_protocol.json and docs/SLA_SENSITIVITY_PROTOCOL.md before new outcomes; freeze commit b85816c, canonical protocol hash ac61d6e30a9b19823bf8ff595feaf7bdf935ebd72206a712ab001e4968dc3f8b. Source manifest protects2,459 files, including all formal evidence, reference caches and baseline/core sources. Same Windows AMD64/20-CPU/32-GiB host configuration; bundled Python3.12.14 with existing pinned packages replaces the unavailable3.12.10 launcher.38 adapter source files and HELIX/profile pins verified.

Used only six heterogeneous primary groups and original links, eight fixed SLA tuples and five shifts. Original grids51/53/46/41/49/50 supplied1,450 unique reference points. Reclassified saved complete metrics and actually ran23,200 evaluator calls on the original grid. Gate passed8,700 old-SLA label comparisons and24 Raw/Legacy decision records including reference scores/best sets; all old evaluator output fields except runtime reproduced. SLA relaxation monotonicity passed before refinement.

Workload-level common refinement added24/24/24/11/24/13 intensities across all five candidates:600 unique HELIX points,600 physical attempts, all successful; zero retry/timeout/failure. Reused memory-aware ceiling12,24GiB budget,6GiB reserve and unchanged per-token reservation rule. No intensity-range or scientific-input change. h101/h102/h105 exhausted per-workload budget with12/6/9 unresolved intervals; h103/h104/h106 resolved all observed transitions. Final grids75/77/70/52/73/63. h103 also reached24 additions; no workload exceeded six rounds. Largest unresolved width18.92%, not hidden as convergence.

Final tables contain32,800 paired evaluator rows,16,400 reference classifications,480 semantic capacity rows and96 decisions. Raw O/C across TPOT .3/.6/1.2/2.4/5/10:1/0,1/50,2/0,2/139,23/803,8/600 (each denominator2050). Raw agreement6/6,6/6,5/6,3/6,3/6,0/6; Legacy6/6,6/6,6/6,4/6,3/6,2/6. TPOT5 median gap−95.08% versus−75.00% at10. At5, mean partition loss2.43% versus operating loss94.90%. h102 Raw1.2/2.4 has zero arithmetic operating loss but unsafe output and100% safety-adjusted loss; Legacy fixes these exact recommendations. TTFT4.68 creates no-safe reference/E cases for h102/h106; B=0 quantities stay null. All24 original endpoint decisions/best sets remain unchanged;12 new-SLA semantic decisions change selection or type with refinement.

Published complete tables/sequences, attempt journals, new reference metrics, technical report and16-section Chinese evidence report. Preserved38 no-safe and23 nonmonotone distinct reference candidate/SLA sequences; all right-censor flags zero. Full integrity checks replay grids/priorities/budgets and recompute metrics, labels, losses, fingerprints and denominators.36 tests pass. One new PNG/SVG figure visually reviewed; corrected long y-label overlap and exposed coincident Raw/Legacy markers using documented categorical offsets. No new mechanism trace, profile stress, candidate, model or workload. Assessment: strengthens current story; No further experiments for bounded claims. Historical Stage3 reports remain immutable; publication boundary explicitly follows the Stage4 instruction.


## 2026-09-17 — Stage 5: one-shot remaining-Prefill correction

Protocol/workloads frozen in e3887ca before any correction outputs; source Stage4 commit4e579bba911ed807c42bd51c0c87d1ecabb17133. Isolated journal_baseline_plus.py scales only profiled Prefill compute, retains full intrinsic H and original Raw/Legacy trajectories. No second correction or fitted parameter.

Phase A reused2050 primary plus130 A100 physical records, no old HELIX reruns. Phase B seeds201–204 at source-bin offsets1140/1150/1190/1200 (last wraps to0–39), disjoint from h101–h106 and each other.500 new physical points,500 attempts, all successful, no retry/timeout/failure. All workloads used16 added intensities;37 unresolved common intervals, max41.42%, retained without expansion.

Held-out C225→153 Raw and227→155 Legacy, but O7→17 in both. Winner agreement unchanged14/16 and13/16. Mean absolute gap decreases only2.17 percentage points overall, with worse TPOT5 tails (+100%→+300% max overestimate). Unsafe output count stays1/16 but w202/TPOT5 recommended intensity doubles and violating requests rise1/48→9/48. Mean operating loss44.78%→38.87%; safety and utilization are separate. Phase A local conservative debt1.547523291→.937171729 s still rejects; optimistic completion drift persists; h101 reversal resolves on the saved grid, with other selection regressions retained.

41 tests pass,5867 protected files unchanged, full baseline equivalence and old96 decisions/480 capacities verified. Two figures visually checked. Final positioning B: retain JB1 as main object, JB1+ as mechanism-guided correction trade-off evidence. See JB1_PLUS_EVIDENCE_REPORT.md and docs/JB1_PLUS_TECHNICAL_REPORT.md. Stop: no JB1++, tuning, expanded matrix or manuscript.


## 2026-09-18 — Stage 6A: JB-Avg-v1 implementation and deterministic validation

Intake HEAD 881f5cbe159dc712178ec413378a3c616a6e739e on main; tracked tree clean. Existing untracked Stage6 metric-SLA artifacts were retained without overwrite or resumption. Protected manifest captures 11,859 pre-existing files, including both JB1 sources, old results, profiles, adapter/HELIX sources and the paused Stage6 package. The latter has 2,065 saved point records (2,050 reused, 15 new completed before this task) and five historical interrupted running attempt entries. It is archived as incomplete work, not a Stage6A experiment.

Added independent jb_avg_v1.py, standard_metrics_v1.py, frozen design JSON, read-only cache migration, deterministic validation script and tests. Execution takes no SLA thresholds, advances Prefill normalized work and Decode token work with one serial mixed-cohort rate, charges stage batches/boundary transfers once, and separates PrefillDone from FirstToken. q=16 governs conservative KV context only. No H, 5ms accounting time, common gamma, SLA bandwidth budget or partition search enters the core. The pure postclassifier uses standard TTFT + average post-first TPOT with ceil(90%*N) joint passes; m=1 checks TTFT only. Unsupported domains and hard-resource infeasibility have explicit, separate statuses.

29 new tests pass, including frozen A-P cases, and all 75 repository regression tests pass. Small CPU runtime sanity (5 warmups / 50 measured each): median 0.0801ms for 1 request/33 outputs, 2.0481ms for 8 staggered requests/264 outputs; no GPU speedup or formal runtime claim. All protected bytes remain unchanged.

Cache audit scans 10,249 compressed files, identifies 3,428 reference records and deduplicates 3,184 physical executions. All 3,184 recover (100%): 101,817 request rows, one m=1, no missing overhead/metrics, no drain/length/max-list/duplicate-conflict/final-completion failures. 3,169 points predate Stage6; 15 belong to its paused supplement. The known Stage4+Stage5 heterogeneous inventory has 2,550 points, counted read-only. Each record's fixed+queue overhead is used; current values all happen to be .005s. Maximum final-time reconstruction error 9.094947017729282e-13s. No old cache needed exclusion; unsupported/corrupt-record behavior is covered by fixtures.

Report: docs/JB_AVG_V1_IMPLEMENTATION_REPORT.md. Decision READY_FOR_FORMAL_VALIDATION, with no model implementation contradiction detected. New HELIX runs=0; formal matrix calls=0; no refinement, ranking, sweep, stress, manuscript or AICCC archaeology. Standard semantics supersede historical primary metric wording prospectively without relabeling old results. Accepted partitioning prior-work boundaries remain explicit. Stop after commit/push and clean synchronized main; no formal experiment is automatically authorized.
