...
def main():
 ...
 for kind,regime in categories:
  ...
  ax.scatter([r['reference']['largest_safe'] for r in rs],[r['evaluator']['largest_safe'] for r in rs],label=kind+' / '+sla_label(regime),alpha=.7,s=30)
 ...
 fig,ax=plt.subplots(figsize=(12,6.5));im=ax.imshow(array,vmin=0,vmax=1,cmap='YlGnBu',aspect='auto');fig.colorbar(im,ax=ax,label='Selected candidate capacity / best tested capacity')
 ax.set_xticks(range(len(variants)),[v.replace('_minus',' −').replace('nominal','Nominal') for v in variants],rotation=45,ha='right');ax.set_yticks(range(len(trials)),[g.split('-')[0]+' / '+sla_label(r) for g,r in trials],fontsize=8);ax.set_title('Common-grid decision transfer\nNominal: S = strict best-set mismatch; T = tie-break mismatch')
 for i in range(len(trials)):
  for j in range(len(variants)):
   if np.isfinite(array[i,j]):
    suffix=''
    if j==0:
     d=lookup[(*trials[i],'nominal')]
     if d['winner_agrees'] is False:suffix=' T' if set(d['evaluator_best_set'])&set(d['reference_best_set']) else ' S'
    ax.text(j,i,f'{array[i,j]:.2f}'+suffix,ha='center',va='center',fontsize=7,color='white' if array[i,j]>.7 else 'black')
 save(fig,'03_decision_stress')
 fig,axes=plt.subplots(1,2,figsize=(12,5));names=CONFIG['mitigations'];labels=['E only','25% compute margin','20%-target, grid snapped','4 scenarios','Point replay','Boundary replay ≤5']
 fig.suptitle('Conditional reference-validation replay on a preconstructed grid\nReference-informed grid construction cost excluded',fontsize=11)
 ...
 write_json(folder/'manifest.json',{'protocol_hash':PROTOCOL_HASH,'figures':['01_reliability','02_capacity','03_decision_stress','04_mitigation'],'notes':['Counts describe the frozen adaptive sample, not population risk.','Capacity scatter omits all-unsafe configurations; full tables retain them.','Decision quality uses the largest observed safe load even for nonmonotone sequences; see flags in tables.','Nominal relaxed-TPOT: four strict best-set mismatches (S), two tie-break mismatches (T).','Mitigation shows 12 main five-candidate trials only; line is mean over defined qualities, dots are individual trials.','Conditional replay uses a preconstructed reference-informed grid; at most five replay queries do not include grid construction or solve planning from scratch.','Derating is a 20% target with grid snapping, not a fixed realized 20% reduction.']})
