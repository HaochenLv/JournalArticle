"""Stage 4 helpers; all writes are confined to the sensitivity directory."""
from pathlib import Path
import sys, json, gzip, math, hashlib
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'src'))
from formal_core import *
OUT=ROOT/'results/sla_sensitivity'
P=json.loads((ROOT/'config/sla_sensitivity_protocol.json').read_text())
PH=fingerprint(P)
...
def guard():
    ...
    assert hashlib.sha256((ROOT/'config/sla_sensitivity_protocol.json').read_bytes()).hexdigest()==snap['config_sha256']
