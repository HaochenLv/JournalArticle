...
**Completed on2026-09-17.** All frozen experiments, postprocessing, formal verification, sequential host timing and mechanism review are finished. See `FORMAL_EVIDENCE_REPORT.md` and `CURRENT_STATUS.md`. Do not restart drivers or rerun completed experiments. The commands below are retained for reproducibility and historical continuation; automatic monitoring is paused after final delivery.
...
Current desktop scheduling (supersedes earlier8/22GiB and fixed4 settings): `scripts/formal_run.py --workers 12 --memory-budget-gib 24 --memory-reserve-gib 6`. The user requested further acceleration; this enables up to12 tasks with per-workload estimated peak reservations, a live physical/commit-headroom admission guard and rotation across physical groups. Actual concurrency varies with workload size and available memory. The estimator is512MiB +0.30MiB per output token, rounded up to256MiB; it is a scheduling estimate, not an OS memory limit. Inspect `.private/memory_scheduler.json` and the log locations in `results/formal/desktop_resume.json`. Do not start a second driver to fill idle slots. The5-minute heartbeat follows the same settings. See `desktop_parallel12_change.json` for the latest execution-resource amendment and1007 preserved checkpoint rows. Frozen scientific inputs and600s/one retry remain unchanged; postprocessing stays at8 workers and formal timing stays sequential.
