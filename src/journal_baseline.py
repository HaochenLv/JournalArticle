"""Recovered journal baseline JB1, specified from paper equations and public E31.

This is NOT the unavailable CA final source. Decisions not specified by the
manuscript (blocking charge and activation buffers) are explicit below.
"""
...
def evaluate(pipeline,workload,sla,prof=None,*,intrinsic=True,trace=False,drain=False,
 ...
 """Full compute governs progress; all overhead belongs only to the ledger.
 ...
 def check(side,events):
  ...
  # The manuscript gives no unique debt recurrence. Freeze the public E31
  # active-Prefill full-service charge, including independently calibrated H.
  debt=sum(x.prefill_compute+H*x.spec.input_tokens for x in active.values() if x.phase=='prefill')
