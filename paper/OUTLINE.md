...
## Stage 4 supplement: SLA-regime sensitivity

Place a controlled SLA-sensitivity subsection after nominal decision/loss results and implementation-semantic sensitivity, before profile stress. Use [SLA_SENSITIVITY_EVIDENCE_REPORT.md](../SLA_SENSITIVITY_EVIDENCE_REPORT.md) for the eight fixed TTFT/TPOT conditions on the same six Stage 3 workloads, shared-grid refinement, Raw/Legacy decisions and exact recommendation safety. This is a post-Stage-3 parameter supplement, not a new independent held-out evaluation; 48 workload/SLA conditions are not 48 independent workloads. Keep one new decision-type/loss figure, with full judgment/capacity and TTFT-tightening tables in the supplement. Report unresolved resolution and any old-endpoint grid sensitivity without replacing Stage 3 history.

For this supplement, describe JB1 as the explicitly specified lightweight evaluator and Legacy as the implementation-semantic control. The withdrawn unpublished AICCC manuscript is not prior publication. Distinguish the accepted *Evaluator-Guided SLA-Aware Layer Partitioning for Heterogeneous LLM Pipelines* as prior work; the five-shift family, evaluator-as-score and partition search are inherited, while the new evidence here is SLA-regime sensitivity. No manuscript prose or new algorithm is added.
